...
function richSave(id, el) {
  ...
  if (def) {
    ...
    if (winner) {
      winner.innerHTML = typeof winnerInner === 'function' ? winnerInner(w) : def.render(w);
...
/** Text/Silbentext: eigener Innenabstand aktiv? (sonst .winner-Padding wie bisher) */
function textUsesInnerPad(w) {
  return w && (w.type === 'text' || w.type === 'silbentext') && w.innerPad != null;
}
