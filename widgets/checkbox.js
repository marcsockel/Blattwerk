...
WIDGETS.push({
  ...
  createData: id => ({ id, type:"checkbox", items:["Richtig","Falsch","Vielleicht"], font:"inherit", fontSize:14, aufgabenNr:0, aufgabenText:''}),
  render: d => {
    const font     = d.font     || "inherit";
    const fontSize = d.fontSize || 14;
    return atHtml(d) + d.items.map(it=>`<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;font-family:${font};font-size:${fontSize}px;"><div style="width:15px;height:15px;border:2px solid #999;border-radius:2px;flex-shrink:0;"></div>${esc(it)}</div>`).join("");
  },
  renderProps: d => {
    const font     = d.font     || "inherit";
    const fontSize = d.fontSize || 14;
