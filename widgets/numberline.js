...
WIDGETS.push({
  ...
  render: d => {
    ...
    if (modus === 'ohne') {
      ...
      return atHtml(d) + `<svg width="100%" viewBox="0 0 ${VW} 62" style="display:block;">
    ...
    if (modus === 'luecken') {
      ...
      return atHtml(d) + `<svg width="100%" viewBox="0 0 ${VW} 70" style="display:block;">
        <line x1="${lineL}" y1="${lineY}" x2="${lineR}" y2="${lineY}" stroke="#000" stroke-width="2"/>
        <polygon points="${lineR+7},${lineY} ${lineR},${lineY-3} ${lineR},${lineY+3}" fill="#000"/>
        ${ticks}${labels}</svg>`;
    }
    ...
    marked.forEach((val, idx) => {
      ...
      if (modus === 'eintragen') {
        // Schreibkästchen 1,5× größer (mehr Platz zum Eintragen)
