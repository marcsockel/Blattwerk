  ...
  render: d => {
    ...
    const items=d.words.split(",").map((w,i)=>{
      ...
      const answerLine = isActive
        ? `<div style="border-bottom:1.5px solid #999;flex:1;height:20px;display:flex;align-items:center;padding-left:6px;color:#2563eb;font-weight:700;font-size:16px;">${esc(u)}</div>`
        : `<div style="border-bottom:1.5px solid #999;flex:1;height:20px;"></div>`;
      return`<div style="display:flex;align-items:center;gap:10px;margin-bottom:7px;font-size:13px;"><span style="font-family:monospace;font-size:14px;background:#f0eee8;padding:3px 8px;border-radius:4px;letter-spacing:2px;">${s}</span><span style="color:#bbb;">→</span>${answerLine}</div>`;}).join("");
