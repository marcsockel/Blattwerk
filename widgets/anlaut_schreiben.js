// Widget: Anlaut-Schreiben
// Anlautbild | Schreiblinie (ohne Wortkarten)
WIDGETS.push({
  meta: {
    type: "anlautschreiben",
    group: "anlaut",
    label: "Anlaut-Schreiben",
  ...
  render: d => {
    ...
    const rows = items.map((item, i) => {
      ...
      return imgCell(item, isFirst ? `asleft-${d.id}` : null) +
        `<div></div>` +
        writeLine(answer);
    }).join("");
  ...
  renderProps: d => {
    ...
    const available = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("").concat(["Ä","Ö","Ü","ß","Ei","Sch","Sp","St"]);

    const letterBtns = available.map(l => {
      const isLong = l.length > 1;
      return `<button onclick="event.stopPropagation();anlautSchreibenAddItem(${d.id},'${l}')"
    ...
    const itemCells = items.map((item, idx) => {
      ...
      const picker   = variants.map(v => {
        const active = v === src;
        return `<button onclick="event.stopPropagation();anlautSchreibenSetItemSrc(${d.id},${idx},'${v}')"
      ...
      return `
        ...
        <button onclick="event.stopPropagation();anlautSchreibenRemoveItem(${d.id},${idx})"
    ...
    return `<div class="prow" style="margin-bottom:4px;">
      ...
      (items.length ? `<div class="prow">
        ...
        <button onclick="event.stopPropagation();anlautSchreibenShuffleItems(${d.id})"
      ...
      `<div class="prow"><label>Größe</label>
        <div style="display:flex;gap:4px;">
          ${toggleBtn("Klein", d.modus !== "gross", `anlautSchreibenModus(${d.id},'klein')`)}
          ${toggleBtn("Groß",  d.modus === "gross", `anlautSchreibenModus(${d.id},'gross')`)}
        </div></div>` +
      pr("Bildgröße (px)",
...
// ── Anlaut-Schreiben helpers ───────────────────────────────────────────────

function anlautSchreibenAddItem(id, anlaut) {
...
function anlautSchreibenRemoveItem(id, idx) {
  const w = widgets.find(x => x.id === id); if (!w) return;
  saveHistory();
...
function anlautSchreibenSetItemSrc(id, idx, src) {
  const w = widgets.find(x => x.id === id); if (!w) return;
  saveHistory();
  const items = [...(w.items || [])];
...
function anlautSchreibenShuffleItems(id) {
  const w = widgets.find(x => x.id === id); if (!w) return;
  saveHistory();
  const items = [...(w.items || [])];
...
function anlautSchreibenModus(id, modus) {
  const w = widgets.find(x => x.id === id); if (!w) return;
  saveHistory();
  w.modus = modus;
